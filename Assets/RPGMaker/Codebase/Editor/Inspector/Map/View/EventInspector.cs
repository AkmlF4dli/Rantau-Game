...
namespace RPGMaker.Codebase.Editor.Inspector.Map.View
{
    ...
    public class EventInspector : AbstractInspectorElement
    {
        ...
        private Toggle _eventTileImageToggle;
        private VisualElement _eventTileImageArea;
        ...
        private VisualElement _eventTileImageContainer;
        private Image _eventTileImage;
        ...
        /// <summary>
        ///     マップイベントページ情報によりマップタイルに表示するスプライトを取得する。
        /// </summary>
        /// <param name="eventMapPage">マップイペントページ情報</param>
        /// <returns>テクスチャー</returns>
        public static Sprite LoadMapTileSprite(EventMapDataModel.EventMapPage eventMapPage) {
            switch (eventMapPage.condition.image.Enabled)
            {
                case EnabledType.Character:
                {
                    ...
                    var texture = assetId != null ? ImageManager.LoadSvCharacter(assetId) : null;
                    return texture ? ImageUtility.SpriteCreate(texture, new Rect(0, 0, texture.width, texture.height), new Vector2(0.5f, 0.5f), 1) : null;
                }

                case EnabledType.SelectedImage:
                    {
                        var texture = LoadSdCharacterTexture(eventMapPage);
                        return texture ? ImageUtility.SpriteCreate(texture, new Rect(0, 0, texture.width, texture.height), new Vector2(0.5f, 0.5f), 1) : null;
                    }

                case EnabledType.Tile:
                    return LoadTileSprite(eventMapPage);
            }
        ...
        public static Sprite LoadTileSprite(EventMapDataModel.EventMapPage eventMapPage) {
            return ImageManager.LoadTile(eventMapPage.image.tileName);
        }
        ...
        override protected void InitializeContents() {
            ...
            int selfSwitchIndex() {
                var index = 0;
                foreach(var v in _selfSwitchDictionary.Values)
                {
                    if( v == _targetEventMapPage.condition.selfSwitch.selfSwitch)
                    {
                        return index;
                    }
                    index++;
            ...
            _eventTileImageToggle = RootContainer.Query<Toggle>("event_tile_image_toggle");
            _eventTileImageArea = RootContainer.Query<VisualElement>("event_tile_image_area");
            ...
            _eventTileImageContainer = RootContainer.Query<VisualElement>("event_tile_image");
        ...
        private void SetEntityToUi() {
            ...
            _eventTileImageToggle.value = enabledType == EnabledType.Tile;
            ...
            _eventTileImageArea.SetEnabled(_eventTileImageToggle.value);
        ...
        private void SetCallbackToUi() {
            ...
            _eventCharacterImageToggle.RegisterValueChangedCallback(o =>
            {
                ...
                if (o.newValue)
                {
                    ...
                    _eventTileImageToggle.value = false;
                }
            ...
            // 『タイル』トグル。
            _eventTileImageToggle.RegisterValueChangedCallback(o =>
            {
                _eventTileImageArea.SetEnabled(o.newValue);
                if (o.newValue)
                {
                    _eventCharacterImageToggle.value = false;
                    _targetEventMapPage.image.name = "";
                ...
                if (_eventCharacterImageToggle.value || _eventSelectImageToggle.value) return;
                _targetEventMapPage.condition.image.Enabled =
                    o.newValue ? EnabledType.Tile : EnabledType.None;
                MapEditor.MapEditor.SaveEventMap(_eventMapDataModel);
                _element.Refresh();
            });
            ...
            // タイル画像ボタン。
            {
                var tileImageButton = _eventTileImageContainer as Button;
                _eventTileImage = new Image();
                _eventTileImage.style.alignSelf = Align.Center;
                _eventTileImageContainer.Add(_eventTileImage);

                if (!string.IsNullOrEmpty(_targetEventMapPage.image.tileName))
                {
                    _eventTileImageContainer.style.justifyContent = Justify.Center;
                    _eventTileImage.sprite = LoadTileSprite(_targetEventMapPage);
                }

                tileImageButton.clickable.clicked += () => { SelectedTile(); };
            }
        ...
        private void SelectedSd() {
            var sdSelectModalWindow = AssetPicker.Instantiate();
            sdSelectModalWindow.CharacterSdType = AssetPicker.CharacterType.Map;
        ...
        private void SelectedTile() {
            ...
            if (_targetEventMapPage.image.tileName?.Length >= 2)
            {
                type = (_targetEventMapPage.image.tileName.StartsWith("0,") ? TileDataModel.Type.NormalTile : TileDataModel.Type.LargeParts);
                tileId = _targetEventMapPage.image.tileName.Substring(2);
            }

            var mapManagementService = Editor.Hierarchy.Hierarchy.mapManagementService;
            var tileEntities = mapManagementService.LoadTileTable();

            var window = TileSelectionWindow.CreateInstance();
            window.Init(tileEntities, tileDataModel =>
            {
                _targetEventMapPage.image.tileName = (tileDataModel == null) ? "" : $"{(tileDataModel.type == TileDataModel.Type.NormalTile ? "0" : "1")},{tileDataModel.id}";
