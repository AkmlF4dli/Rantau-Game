...
namespace RPGMaker.Codebase.Editor.Inspector.Option.View
{
    ...
    public class OptionInspectorElement : AbstractInspectorElement
    {
        ...
        override protected void InitializeContents() {
            ...
            //タイトル画面をスキップ(テストプレイ)
            Toggle option7 = RootContainer.Query<Toggle>("option_7");
            option7.value = _systemSettings.optionSetting.optSkipTitleScreen == 1
                ? true
                : false;
            option7.RegisterValueChangedCallback(evt =>
            {
                _systemSettings.optionSetting.optSkipTitleScreen =
                    option7.value ? 1 : 0;
                Save();
            });

            //U309 オプション画面で、常時ダッシュの非表示設定
            Toggle option8 = RootContainer.Query<Toggle>("option_8");
            option8.value = _systemSettings.optionSetting.optDashHide == 1
                ? true
                : false;
            option8.RegisterValueChangedCallback(evt =>
            {
                _systemSettings.optionSetting.optDashHide = option8.value ? 1 : 0;
                Save();
            });
