...
namespace RPGMaker.Codebase.Editor.Inspector.Effect.View
{
    ...
    public class EffectInspectorElement : AbstractInspectorElement
    {
        ...
        public void SetEffects(
            ...
            for (var i = 0; i < skillCustomDataModels.Count; i++) skillUidList.Add(skillCustomDataModels[i].basic.id);
            ...
            //U261 コモンイベントUIDリストに保存
            var commonuidList = new List<string>();

            foreach (var c in commons)
            {
                commonList.Add(c.name);
                //U261 コモンイベントUIDリストに保存
                commonuidList.Add(c.eventId);
            }
            ...
            else if (_trait[index].categoryId == (int) EffectCategoryEnum.OTHER)
            {
                ...
                else if (_trait[index].traitsId == (int) EffectOtherEnum.MASTER_SKILL)
                {
                    ...
                    //U261 スキルUIDから判定する
                    if (skillUidList.Contains(_trait[index].effectUID))
                    {
                        _trait[index].effectId = skillUidList.LastIndexOf(_trait[index].effectUID);
                    }
                    else
                    {
                        //指定データなし…
                        _trait[index].effectId = 0;
                        if (skillUidList.Count != 0)
                        {
                            //先頭のデータを指定
                            _trait[index].effectUID = skillUidList[0];
                        }
                        else
                        {
                            _trait[index].effectUID = "";
                        }
                    }
                }
                else if (_trait[index].traitsId == (int) EffectOtherEnum.COMMON_EVENT)
                {
                    ...
                    //U261 コモンイベントUIDから判定する
                    if (commonuidList.Contains(_trait[index].effectUID))
                    {
                        _trait[index].effectId = commonuidList.LastIndexOf(_trait[index].effectUID);
                    }
                    else
                    {
                        //指定データなし…
                        _trait[index].effectId = 0;
                        if (commonuidList.Count != 0)
                        {
                            //先頭のデータを指定
                            _trait[index].effectUID = commonuidList[0];
                        }
                        else
                        {
                            _trait[index].effectUID = "";
            ...
            effectDropdownPopupField.RegisterValueChangedCallback(evt =>
            {
                ...
                else if (_trait[index].categoryId == (int) EffectCategoryEnum.OTHER)
                {
                    ...
                    else if (_trait[index].traitsId == (int) EffectOtherEnum.MASTER_SKILL)
                    {
                        ...
                        //U261 UIDを保存
                        _trait[index].effectUID = skillUidList[effectDropdownPopupField.index];
                    }
                    else if (_trait[index].traitsId == (int) EffectOtherEnum.COMMON_EVENT)
                    {
                        _trait[index].effectId = effectDropdownChoices.LastIndexOf(effectDropdownPopupField.value);
                        //U261 UIDを保存
                        _trait[index].effectUID = commonuidList[effectDropdownPopupField.index];
                    }
                }
