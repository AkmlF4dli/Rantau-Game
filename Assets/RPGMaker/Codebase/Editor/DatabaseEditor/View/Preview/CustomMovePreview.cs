    ...
    public class CustomMovePreview : AbstractPreview
    {
        ...
        protected override string GetAssetId() {
            ...
            // このイベント
                ...
                if (eventManage.LoadEventMap().Find(data => data.eventId == _eventId).pages[0].condition.image.enabled == (int) EnabledType.SelectedImage)
                    assetId = eventManage.LoadEventMap().Find(data => data.eventId == _eventId).pages[0].image.sdName;
                else if (eventManage.LoadEventMap().Find(data => data.eventId == _eventId).pages[0].condition.image.enabled == (int) EnabledType.Character)
                ...
                else if (eventManage.LoadEventMap().Find(data => data.eventId == _eventId).pages[0].condition.image.enabled == (int) EnabledType.Tile)
                    assetId = eventManage.LoadEventMap().Find(data => data.eventId == _eventId).pages[0].image.tileName;
            // 指定イベント
            else if (eventManage.LoadEventMap().Find(data => data.eventId == _targetId) != null)
                if (eventManage.LoadEventMap().Find(data => data.eventId == _targetId).pages[0].condition.image.enabled == (int) EnabledType.SelectedImage)
                    assetId = eventManage.LoadEventMap().Find(data => data.eventId == _targetId).pages[0].image.sdName;
                else if (eventManage.LoadEventMap().Find(data => data.eventId == _targetId).pages[0].condition.image.enabled == (int) EnabledType.Character)
                    ...
                    else if (eventManage.LoadEventMap().Find(data => data.eventId == _targetId).pages[0].condition.image.enabled == (int) EnabledType.Tile)
                        assetId = eventManage.LoadEventMap().Find(data => data.eventId == _targetId).pages[0].image.tileName;
