...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.CommandEditor.Character.Image
{
    ...
    public class CharacterImage : AbstractCommandEditor
    {
        ...
        private ImageType _imageType = ImageType.Character;
        private int _num = 0;
        ...
        public override void Invoke() {
            ...
            _imageType = ImageType.Character;
            if (_targetCommand.parameters[1].StartsWith("0,") || _targetCommand.parameters[1].StartsWith("1,"))
            {
                _imageType = ImageType.Tile;
            } else
            ...
            var characterType = EditorLocalize.LocalizeTexts(new List<string> { "WORD_0350", "WORD_1326", "WORD_1565" });
            VisualElement characterTypeSelectort = RootElement.Query<VisualElement>("character_image_type");
            var characterTypePopupField = new PopupFieldBase<string>(characterType, (int) _imageType);
            ...
            {
                if (characterTypePopupField.index == 0 && _imageType != ImageType.Character)
                {
                    _imageType = ImageType.Character;
                }
                else if (characterTypePopupField.index == 1 && _imageType != ImageType.Object)
                {
                    _imageType = ImageType.Object;
                }
                else if (characterTypePopupField.index == 2 && _imageType != ImageType.Tile)
                {
                    _imageType = ImageType.Tile;
                }
                else
                ...
                characterImagePreview.sprite = LoadSprite(_targetCommand.parameters[1]);

                //タイル画像の場合は「向き」の項目を非表示にする。
                VisualElement directionItem = RootElement.Query<VisualElement>("directionItem");
                directionItem.style.display = (_imageType == ImageType.Tile) ? DisplayStyle.None : DisplayStyle.Flex;
                
                if (_imageType != ImageType.Tile)
                {
                    ...
                    if (_imageType == ImageType.Character) directionTextDropdownChoices.Add(EditorLocalize.LocalizeText("WORD_0509"));

                    if (_imageType != ImageType.Character && _targetCommand.parameters[7] == "5")
            ...
            if (_imageType == ImageType.Object && _targetCommand.parameters[7] == "5")
                _targetCommand.parameters[7] = "1";

            //タイル画像の場合は「向き」項目を非表示にする。
            VisualElement directionItem = RootElement.Query<VisualElement>("directionItem");
            directionItem.style.display = (_imageType == ImageType.Tile) ? DisplayStyle.None : DisplayStyle.Flex;
        ...
        private void CreatePopupField() {
            ...
            PopupFieldBase<string> characterImagePopupField = null;

            if (_imageType == ImageType.Tile)
            {
                var button = new Button();
                button.text = EditorLocalize.LocalizeText("WORD_5034");
                button.clickable.clicked += () =>
                {
                    var type = TileDataModel.Type.NormalTile;
                    string tileId = string.Empty;
                    if (_targetCommand.parameters[1].Length >= 2)
                    {
                        type = (_targetCommand.parameters[1].StartsWith("0,") ? TileDataModel.Type.NormalTile : TileDataModel.Type.LargeParts);
                        tileId = _targetCommand.parameters[1].Substring(2);
                    }
                    ...
                    {
                        _targetCommand.parameters[1] = (tileDataModel == null) ? "" : $"{(tileDataModel.type == TileDataModel.Type.NormalTile ? "0" : "1")},{tileDataModel.id}";
                        characterImagePreview.sprite = LoadSprite(_targetCommand.parameters[1]);
                ...
                characterImageSelector.Add(button);
            }
            else
            {
                //なしを追加
                ...
                //キャラクター
                if (_imageType == ImageType.Character)
                {
                ...
                //オブジェクト
                else if (_imageType == ImageType.Object)
                {
                    characterImageSelectorChoices.Clear();
                    characterImageSelectorChoices.Add(EditorLocalize.LocalizeText("WORD_0113"));
                    characterIdSelectorChoices.Add("-1");
                ...
                //セレクトボックス作成
                characterImagePopupField = new PopupFieldBase<string>(characterImageSelectorChoices, _num);
        ...
        private Sprite LoadSprite(string fileName) {
            if (_imageType == ImageType.Tile)
            {
                return ImageManager.LoadTile(fileName);
            }
            var imageCharacter = ImageManager.LoadSvCharacter(fileName);

            return imageCharacter ? ImageUtility.SpriteCreate(imageCharacter, new Rect(0.0f, 0.0f, imageCharacter.width, imageCharacter.height), new Vector2(0.5f, 0.5f), 100.0f) : null;
        }
    }
}