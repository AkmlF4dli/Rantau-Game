    ...
    public class ConditionalBranch : AbstractCommandEditor
    {
        ...
        public override void Invoke() {
            ...
            if (selectID == -1)
            {
                selectID = 0;
                //U396初期値設定
                _targetCommand.parameters[7] = variableDropdownChoices[0];
            }
            ...
            if (selectID == -1)
            {
                selectID = 0;
                //U396初期値設定
                _targetCommand.parameters[11] = variableDropdownChoices[0];
            }
            ...
            var pushed = EditorLocalize.LocalizeTexts(new List<string>
                {"WORD_1163", "WORD_1164", "WORD_1165", "WORD_6107"});
        ...
        private void AndOrProcess() {
            ...
            if (_targetCommand.parameters[1] == "0")
                return;
            // 設定する値
            var targetCode = (_targetCommand.parameters[2] == "0")? EventEnum.EVENT_CODE_FLOW_AND : EventEnum.EVENT_CODE_FLOW_OR;
            for (var i = EventCommandIndex + 1; i < EventDataModels[EventIndex].eventCommands.Count; i++)
            {
                var eventCommand = EventDataModels[EventIndex].eventCommands[i];
                // AND/ORのコードの場合上書きする
                if (eventCommand.code == (int) EventEnum.EVENT_CODE_FLOW_OR ||
                    eventCommand.code == (int) EventEnum.EVENT_CODE_FLOW_AND)
                {
                    eventCommand.code = (int) targetCode;
                } else
                {
                    // それ以外のコードが出てきた場合、条件分岐のスコープを抜けているので終了する
                    break;
                }
            }
        }
        ...
        private void EditEvent(FlowKind toggleNum, List<string> parameters) {
                ...
                i++)
            {
                //編集するのがAndかOrのイベントか
                var eventcommand = EventDataModels[EventIndex].eventCommands[i];
                if (eventcommand.code == (int) EventEnum.EVENT_CODE_FLOW_AND ||
                    eventcommand.code == (int) EventEnum.EVENT_CODE_FLOW_OR)
                {
                    if (eventcommand.parameters[0] == ((int) toggleNum).ToString())
                    {
                        eventcommand.parameters = parameters;
                        break;
                    }
                }
                else
