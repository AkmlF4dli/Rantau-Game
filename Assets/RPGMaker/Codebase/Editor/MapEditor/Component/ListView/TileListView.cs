...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.ListView
{
    ...
    public class TileListView : ScrollView
    {
        ...
        private static readonly Color BORDER_COLOR = Color.red;
        ...
        private static Texture DefaultTexture
        {
            get
            {
                if (m_defaultimage == null)
                    m_defaultimage = UnityEditorWrapper.AssetDatabaseWrapper.LoadAssetAtPath<Texture>("Assets/RPGMaker/Storage/System/Map/loading.png");
                return m_defaultimage;
            }
        }
        private static bool IsImageIsNowLoadiing(Image image) {
            return image.sprite == null && image.image == null && image.image.GetInstanceID() == DefaultTexture.GetInstanceID();
        }
        ...
        private Action<TileDataModel> _onDoubleClickTile;
        ...
        private TileDataModel _scrollToTile = null;
        private bool _reselecCurrent = false;
        ...
        public void SetDoubleClickTileCallback(Action<TileDataModel> callback) {
            _onDoubleClickTile = callback;
        }

        public void SetReselectCurrent(bool reselecCurrent) {
            _reselecCurrent = reselecCurrent;
        }
        ...
        public void Refresh(
            ...
            var current = CurrentSelectingTile;

            CreateData();

            if (_reselecCurrent)
            {
                if (current != null && _tileEntities.Contains(current.tileDataModelInfo))
                {
                    CurrentSelectingTile = current;
                    if (CurrentSelectingTile != null)
                    {
                        SelectTile(CurrentSelectingTile);
                    }
                } else
                {
                    _currentSelectingTile = current;
                }
            } else
        ...
        private void CreateData() {
            ...
            if (!_reselecCurrent && _tileEntities != null && _tileEntities.Count > 0 && findTile)
            {
        ...
        private void SetTile(Vector2Int position, TileDataModelInfo tileDataModel) {
            ...
            {
                // 画像が読み込まれていなければ処理しない
                if (IsImageIsNowLoadiing(tileImage))
                {
                    return;
                }
            ...
            tileImage.RegisterCallback<MouseUpEvent>(evt =>
            {
                ...
                if (EditorApplication.timeSinceStartup - lastClickTime < clickInterval)
                {
                    // ダブルクリックされたときの処理
                    evt.StopPropagation();
                    _onDoubleClickTile?.Invoke(tileDataModel.TileDataModel);
                }
        ...
        public void SelectAndScrollToTile(TileDataModel tileDataModel) {
            SelectTile(tileDataModel);
            _scrollToTile = tileDataModel;
            this.RegisterCallback<GeometryChangedEvent>(OnScrollViewRendered);
        }

        private void OnScrollViewRendered(GeometryChangedEvent evt) {
            if (_scrollToTile == null) return;
            var tileImage = GetTileImage(_scrollToTile);
            if (tileImage != null)
            {
                ScrollTo(tileImage);
            }
                ...
                while (!flg)
                {
                    foreach (var elem in _rowElements)
                    {
                        ...
                        if (worldBound.y < elem.worldBound.y + TILE_SIZE && worldBound.y + worldBound.height > elem.worldBound.y - TILE_SIZE)
                        {
                            ...
                            {
                                var info = image.userData as TileDataModelInfo;
                                if (info == null)
                                    continue;

                                // 既に読込済みであれば即設定
                                if (info.isLoadedTileDataModel())
                                {
                                    var sprite = info.TileDataModel.m_DefaultSprite;
                                    image.image = null;
                                    image.sprite = sprite;
                                }
                                else
        ...
        private IEnumerator TileDisplay(List<Image> images) {
            ...
            for (int i = 0; i < images.Count; i++)
            {
                ...
                if (IsImageIsNowLoadiing(images[i]) == false)
                {
                    try
                    {
                        images[i].image = null;
                        images[i].sprite = tileDataModel.m_DefaultSprite;
                    }
                    catch (Exception) { }
                }
                count++;
        ...
        private void RemoveCursor(TileDataModel tileDataModel) {
            ...
            if (tileDataModel != null && !_tileEntities.Contains(tileDataModel.tileDataModelInfo))
            {
                return;
            }
