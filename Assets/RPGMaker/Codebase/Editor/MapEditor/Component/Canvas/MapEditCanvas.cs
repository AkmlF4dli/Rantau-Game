...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.Canvas
{
    ...
    public class MapEditCanvas : MapCanvas
    {
        ...
        private bool _isShadowDraw;     // If true, draw a shadow. If false, erase the shadow.
        private AutoShadowPaint _autoShadowPaint;
        private readonly HashSet<MapDataModel.Layer.LayerType> AutoShadowLayerSet = new HashSet<MapDataModel.Layer.LayerType>() {
        ...
        private void Brush(Vector3Int cellPos) {
            ...
            switch (_drawMode)
            {
                ...
                case DrawMode.Shadow:
                    _layers[(int) MapDataModel.Layer.LayerType.Shadow].tilemap.SetTile(cellPos, _isShadowDraw ? _shadowToDraw : null);
                    break;
                case DrawMode.Delete:
                    _currentTargetMapLayer.tilemap.SetTile(cellPos, null);
                    _autoShadowPaint?.AddCell(cellPos);
                    break;
        ...
        private void Fill(Vector3Int cellPos, TileDataModel fillTarget) {
            ...
            if (_autoShadowPaint != null)
            {
                var lastTiles = new TileDataModel[MapDataModel.width, MapDataModel.height];
                for (int x = 0; x < MapDataModel.width; x++)
                {
                    for (int y = 0; y < MapDataModel.height; y++)
                    {
                        lastTiles[x, y] = _currentTargetMapLayer.GetTileDataModelByPosition(new Vector2(x, -y));
                    }
                }
                //U393参照切れ対策
                _currentTargetMapLayer.tilemap.FloodFill(cellPos, null);
                _currentTargetMapLayer.tilemap.FloodFill(cellPos, _currentTileToDraw);
                for (int x = 0; x < MapDataModel.width; x++)
                {
                    for (int y = 0; y < MapDataModel.height; y++)
                    {
                        var tileDataModel = _currentTargetMapLayer.GetTileDataModelByPosition(new Vector2(x, -y));
                        if (tileDataModel != lastTiles[x, y])
                        {
                            _autoShadowPaint?.AddCell(new Vector3Int(x, -y, 0));
                        }
                    }
                }
            }
        ...
        private void RectangleDelete(Vector3Int cellPos) {
            ...
            foreach (var location in bounds.allPositionsWithin)
            {
                pos.Add(location);
                tile.Add(null);
                _autoShadowPaint?.AddCell(location);
            }
        ...
        private void EllipseBrush(Vector3Int cellPos) {
            ...
            for (var y = 0; y < Mathf.Abs(newRect.height); y++)
                for (var x = 0; x < Mathf.Abs(newRect.width); x++)
                {
                    ...
                    var cp = new Vector3Int(newRect.x + x * signX, newRect.y + y * signY, 0);
                    if (xx / aa + yy / bb < 1)
                    {
                        if (_isSpoitDrawMode)
                        {
                        ...
                        SetTile(cp, _currentTileToDraw);
                        _autoShadowPaint?.AddCell(cp);
                    }
                    else
                    {
                        _currentTargetMapLayer.tilemap.SetTile(cp, null);
                    }
                }
        }
        ...
        public void SetTiles(List<Vector3Int> cellsPos, List<TileDataModel> tileDataModels) {
            for (int i = 0; i < cellsPos.Count; i++)
            {
                ...
                _autoShadowPaint?.AddCell(cellsPos[i]);
            }
        }
        ...
        protected override void OnMouseDown(IMouseEvent e) {
                ...
                cellPos.y <= 1 && (_currentTargetMapLayer.type == MapDataModel.Layer.LayerType.Shadow || _drawMode == DrawMode.Shadow))
            {
                ...
                if (_drawMode == DrawMode.Shadow)
                {
                    var tile = _layers[(int) MapDataModel.Layer.LayerType.Shadow].GetTileDataModelByPosition(new Vector2(cellPos.x, cellPos.y));
                    _isShadowDraw = (tile == null);
                }
                _autoShadowPaint = null;
                if (AutoShadowLayerSet.Contains(_currentTargetMapLayer.type) && !e.shiftKey)
                {
                    _autoShadowPaint = new AutoShadowPaint(MapDataModel, _currentTargetMapLayer, _layers[(int) MapDataModel.Layer.LayerType.Shadow]);
                }
                Brush(cellPos);
            }
        }
        ...
        /// <summary>
        /// タイルセットする前に参照切れ対策(U393対策)
        /// </summary>
        /// <param name="pos"></param>
        private void CheckTile(Vector3Int pos)
        {
            var tileBase = _currentTargetMapLayer.tilemap.GetTile(pos);
            //タイルへの参照が切れている場合にTileクラスが返る
            if(tileBase as Tile)
            {
                _currentTargetMapLayer.tilemap.SetTile(pos,null);
            }
