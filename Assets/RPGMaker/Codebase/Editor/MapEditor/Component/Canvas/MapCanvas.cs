...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.Canvas
{
    ...
    public class MapCanvas : VisualElement, IDisposable
    {
        ...
        protected virtual void Render() {
            ...
            {
                _effectRenderer = new EffectRenderer(RenderTextureCanvasImage);
                _effectRenderer.Start();
            }
            _effectRenderer.RenderTexture = RenderTexture;
        ...
        protected void SetAllEventTiles(EventMapDataModel.EventMapPage targetEventMapPage) {
            ...
            foreach (var eventMapDataModel in _eventMapDataModels)
            {
                ...
                static void PutImage(
                    ...
                    var sprite = EventInspector.LoadMapTileSprite(eventMapPage);
                    if (sprite == null) return;

                    var imageTile = SpriteToMapTile(sprite);
        ...
        public void SetEventTilemapEnabled(bool bEnabled)
        {
            var TileMapObject = _tilemapLayers[(int) TilemapLayer.Type.Event].gameObject;
            TileMapObject.SetActive(bEnabled);
        }
        ...
        private static Tile TextureToMapTile(Texture2D texture) {
            ...
            tile.sprite = ImageUtility.SpriteCreate(
                texture,
        ...
        private static Tile SpriteToMapTile(Sprite sprite) {
            if (sprite == null) return null;
            var scale = (float) TileRepository.TileDefaultSize / Math.Max(sprite.rect.width, sprite.rect.height);
            var pixelsPerUnit = 100.0f / scale;
            var tile = ScriptableObject.CreateInstance<Tile>();
            tile.sprite = ImageUtility.SpriteCreate(
                sprite.texture,
