...
namespace RPGMaker.Codebase.Editor.MapEditor.Window.MapEdit
{
    ...
    public class MapEditWindow : BaseWindow
    {
        ...
        public void CaptureAndSaveMapThumbnail() {
            ...
            //イベントタイルの有効/無効を切替える U341
            _mapEditCanvas.SetEventTilemapEnabled(false);
            ...
            //イベントタイルの有効/無効を切替える U341
            _mapEditCanvas.SetEventTilemapEnabled(true);
            _mapEditCanvas.Refresh();
        ...
        /// <summary>
        /// U258 マップからタイルグループを生成する(グループデータの復元目的)
        /// </summary>
        /// <param name="mapDataModel"></param>
        /// <param name="IsTileGroupAdd"></param>
        public void GenerateTileGroupFromMapData(bool isAddToExistingTileGroup) {
            if (_mapDataModel == null) return;
            GenerateTileGroupFromMapData(_mapDataModel, isAddToExistingTileGroup);
            _mapEditCanvas.Refresh(_mapDataModel);
            _layerInventory.Refresh();
        }
        ...
        /// </summary>
        /// <param name="mapDataModel"></param>
        /// <param name="isAddToExistingTileGroup"></param>
        public static void GenerateTileGroupFromMapData(MapDataModel mapDataModel, bool isAddToExistingTileGroup) {
            //タイルデータ取得
            var tileEntities = Hierarchy.Hierarchy.mapManagementService.LoadTileTable();
            var tilemaps = mapDataModel.LayersForRuntime.ConvertAll(layer => layer.tilemap);
            // マップの表示範囲の設定
            var MapSize = new Vector2Int(mapDataModel.width, mapDataModel.height);
            for (int i = 0; i < tilemaps.Count; i++)
            {
                //影レイヤーはなにもしない
                if (tilemaps[i] == null ||
                    i == (int) MapDataModel.Layer.LayerType.Shadow)
                {
                    continue;
                }

                var tilemap = tilemaps[i];
                var mapDataLayer = mapDataModel.LayersForEditor[i];
                //現在のグループにマップ上の未登録のタイルを追加する
                if (isAddToExistingTileGroup == false)
                {
                    mapDataLayer.tilesOnPalette = new();
                }
                //マップは、縦に－方向で配置されている
                for (var y = -(MapSize.y - 1); y <= 0; y++)
                {
                    for (var x = 0; x < MapSize.x; x++)
                    {
                        //ループ位置のチップを取得する
                        //マップチップを取得
                        var tileData = tilemap.GetTile(new Vector3Int(x, y, 0)) as TileDataModel;
                        if (tileData == null)
                        {
                            continue;
                        }
                        //マップチップが存在しているか?
                        var tileInfo = tileEntities.Find(data => data.TileDataModel == tileData);
                        if (tileInfo == null)
                        {
                            continue;
                        }
                        //グループにマップチップを登録
                        if (mapDataLayer.tilesOnPalette.Contains(tileInfo) == false)
                        {
                            mapDataLayer.tilesOnPalette.Add(tileInfo);
                        }
                    }
                }
            }
