...
namespace RPGMaker.Codebase.CoreSystem.Helper
{
    public class ImageUtility
    {
        ...
        public List<Sprite> SliceImageToSprite(Texture2D texture, int x, int y) {
            try
            {
                ...
                for (var partX = 0; partX < x; partX++)
                {
                    ...
                    partTexture.SetPixels(partPixels);
                    partTexture.Apply();
                    var sp = SpriteCreate(partTexture, new Rect(0, 0, partTexture.width, partTexture.height),
                        Vector2.zero);
                    sliceBitmaps.Add(sp);
        ...
        /// <summary>
        /// Sprite.CreateのSpriteMeshType指定をFullRectに設定する為のラッパー関数
        /// </summary>
        ...
        /// <returns></returns>
        public static Sprite SpriteCreate(Texture2D texture, Rect rect, Vector2 pivot, float pixelsPerUnit = 100.0f)
        {
            var spriteData = Sprite.Create(texture, rect, pivot, pixelsPerUnit, 0, SpriteMeshType.FullRect);
            return spriteData;
        }
    }
}