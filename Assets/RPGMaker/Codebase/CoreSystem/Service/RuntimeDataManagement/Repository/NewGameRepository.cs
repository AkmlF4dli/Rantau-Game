...
namespace RPGMaker.Codebase.CoreSystem.Service.RuntimeDataManagement.Repository
{
    public partial class NewGameRepository
    {
        ...
        private async Task Initialize() {
            ...
            _vehiclesDataModel = await _databaseManagementService.LoadCharacterVehicles();
            _weaponDataModels = await _databaseManagementService.LoadWeapon();
            _armorDataModels = await _databaseManagementService.LoadArmor();
#endif
        }
        ...
        public async Task CreateGame(BattleSceneTransition battleTest = null) {
...
#else
            var flags = await _databaseManagementService.LoadFlags();
...
#else
            for (var i = 0; i < flags.variables.Count; i++)
#endif
...
#else
                RuntimeActorDataModel data = await CreateActorData(actor, battleTest);
#endif
                if (data != null)
            ...
            foreach (var vehicle in _vehiclesDataModel)
            {
                ...
                Vhicle.layerType = (int)vehicle.layerType;
            ...
            //復活ポイントを設定
            RuntimeSaveData.respawnPointData.gameOverScreenFlg = true;
            if (_systemSettingDataModel.initialParty.respwnPointType == 1)
            {
                RuntimeSaveData.respawnPointData.mapID      = _systemSettingDataModel.initialParty.startMap.mapId;
                RuntimeSaveData.respawnPointData.x          = _systemSettingDataModel.initialParty.startMap.position[0];
                RuntimeSaveData.respawnPointData.y          = _systemSettingDataModel.initialParty.startMap.position[1];
                RuntimeSaveData.respawnPointData.eStatus    = RuntimeSaveDataModel.RespawnPointData.EStatus.FULL_RECOVERY;
            }
        }
...
#else
        public async Task<RuntimeActorDataModel> CreateActorData(BattleSceneTransition.Actor actorData, BattleSceneTransition battleTest = null) {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            Initialize();
#else
...
#else
            var actorDataModels = await _databaseManagementService.LoadCharacterActor();
            var classDataModels = await _databaseManagementService.LoadCharacterActorClass();
#endif
...
#else
            var classCommonData = (await new DatabaseManagementService().LoadClassCommon())[0];
#endif
...
#else
            var classDataModel = await new DatabaseManagementService().LoadClassCommonByClassId(actor1.exp.classId);
#endif
...
#else
                                    await ItemManager.ChangeEquipment(actor1, equipTypeData, equip.value, equipIndex, true);
#endif
                                    break;
                                }
                            break;
...
#else
                                    await ItemManager.ChangeEquipment(actor1, equipTypeData, _weaponDataModels[k].basic.id, j, true);
#endif
                                    break;
                                }
...
#else
                                        await ItemManager.ChangeEquipment(actor1, equipTypeData, _armorDataModels[k].basic.id, j, true);
#endif
                                        break;
                                    }
                        }
...
#else
        public async Task<RuntimeSaveDataModel> LoadData(int id) {
#endif
            // Runtimeの処理
            // テキスト読込
            var jsonString =
...
#else
                await UnityEditorWrapper.AssetDatabaseWrapper.LoadJsonString(Application.persistentDataPath + "/" +
#endif
                                                                       "file1.json");
        ...
        public RuntimeConfigDataModel LoadConfig() {
            //U414 起動中にコンフィグクラスの生成を一意にする(各参照が別になるので)
