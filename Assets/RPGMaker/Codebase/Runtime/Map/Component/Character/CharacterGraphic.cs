...
namespace RPGMaker.Codebase.Runtime.Map.Component.Character
{
    public class CharacterGraphic : MonoBehaviour
    {
...
#else
        public virtual async Task Init(string assetId, bool isFirstInit = true) {
#endif
            if (assetId == "")
            {
                //フキダシ用に作成する
            ...
            {
                SetSize(new Vector2(
                    _downDirectionSprite.sprite.rect.width / _downDirectionSprite.animationFrame / TILE_SIZE,
                    _downDirectionSprite.sprite.rect.height / TILE_SIZE));
...
#else
        public virtual async Task ChangeAsset(string assetId) {
#endif
            // 画像が設定されていなければ初期化
        ...
        protected virtual void ChangeTextureSize() {
            ...
            if (spriteData?.sprite?.texture != null)
                SetSize(new Vector2(
                    spriteData.sprite.rect.width / spriteData.animationFrame / TILE_SIZE,
                    spriteData.sprite.rect.height / TILE_SIZE));
        }
...
#else
        protected virtual async Task SetUpSprites(string assetId) {
#endif
            if (assetId != null && (assetId.StartsWith("0,") || assetId.StartsWith("1,")))
            {
                //タイル
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
                ...
                if (assetId.Length >= 2)
                {
                    type = (assetId.StartsWith("0,") ? TileDataModel.Type.NormalTile : TileDataModel.Type.LargeParts);
                    tileId = assetId.Substring(2);
                }
                var tileDataModelInfo = new TileDataModelInfo();
                tileDataModelInfo.type = type;
                tileDataModelInfo.id = tileId;
                var assetPath = $"{TileRepository.GetAssetPath(tileDataModelInfo, true)}{tileDataModelInfo.id}.asset";
                var tileDataModel =
                    UnityEditorWrapper.AssetDatabaseWrapper
                        .LoadAssetAtPath<TileDataModel>(assetPath);
#else
                var tileDataModel =
#if !UNITY_WEBGL
                    AddressableManager.Load.LoadAssetSync<TileDataModel>($"{(assetId.StartsWith("0,") ? "NormalTile" : "LargeParts")}/{assetId.Substring(2)}.asset") as TileDataModel;
#else
                    (await AddressableManager.Load.LoadAssetSync<TileDataModel>($"{(assetId.StartsWith("0,") ? "NormalTile" : "LargeParts")}/{assetId.Substring(2)}.asset")) as TileDataModel;
#endif
#endif
                ...
                //各方向のキャラクター画像
                _down = tileDataModel.m_DefaultSprite;
                _left = tileDataModel.m_DefaultSprite;
                _right = tileDataModel.m_DefaultSprite;
                _up = tileDataModel.m_DefaultSprite;
                _damage = tileDataModel.m_DefaultSprite;
...
#else
                    (await ScriptableObjectOperator.GetClass<AssetManageDataModel>("SO/" + assetId + ".asset", true)) as AssetManageDataModel;
#endif
...
#else
                _down = assetManageData.assetTypeId == (int) AssetCategoryEnum.MOVE_CHARACTER
                            ? await LoadCharacterSprite(assetManageData.imageSettings[0].path)
                            : await LoadObjectSprite(assetManageData.imageSettings[0].path);
                _left = assetManageData.assetTypeId == (int) AssetCategoryEnum.MOVE_CHARACTER
                            ? await LoadCharacterSprite(assetManageData.imageSettings[1].path)
                            : await LoadObjectSprite(assetManageData.imageSettings[1].path);
                _right = assetManageData.assetTypeId == (int) AssetCategoryEnum.MOVE_CHARACTER
                            ? await LoadCharacterSprite(assetManageData.imageSettings[2].path)
                            : await LoadObjectSprite(assetManageData.imageSettings[2].path);
                _up = assetManageData.assetTypeId == (int) AssetCategoryEnum.MOVE_CHARACTER
                            ? await LoadCharacterSprite(assetManageData.imageSettings[3].path)
                            : await LoadObjectSprite(assetManageData.imageSettings[3].path);
                _damage = assetManageData.assetTypeId == (int) AssetCategoryEnum.MOVE_CHARACTER
                            ? await LoadCharacterSprite(assetManageData.imageSettings[4].path)
                            : await LoadObjectSprite(assetManageData.imageSettings[4].path);
        ...
        private Sprite SetBushToSprite(Sprite sp) {
            ...
            return ImageUtility.SpriteCreate(
                texture: texture,
...
#else
        public virtual async Task ReloadCharacterImage(string id) {
#endif
...
#else
        protected virtual async Task<Sprite> LoadCharacterSprite(string fileName) {
#endif
...
#else
        protected virtual async Task<Sprite> LoadObjectSprite(string fileName) {
#endif
            if (string.IsNullOrEmpty(fileName)) return null;
    ...
    static class TextureCopyExtension
    {
        public static Sprite ToSprite(this Texture2D tex) {
            return ImageUtility.SpriteCreate(tex, new Rect(0, 0, tex.width, tex.height), Vector2.zero);
        }
