...
#else
        public async Task Init(RuntimeActorDataModel actor) {
#endif
            //表示データ
...
#else
            GameActor = await DataManager.Self().GetGameActors().Actor(actor);
#endif
            ...
            if (_face != null && _body != null)
            {
                ...
                else if (characterType == (int) MenuIconTypeEnum.SD)
                {
                    ...
                    //パーティウィンドウ以外の場合のみ設定　U227
                    if (transform.GetComponentInParent<PartyWindow>() == null)
                    {
...
#else
                    await SetState(_stateIcons[_iconIndex], GameActor.Actor.states[i].id);
#endif
                    _iconIndex++;
...
#else
        public async Task UpdateData(RuntimeActorDataModel actor) {
#endif
            RuntimeActorDataModel = actor;
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            GameActor = DataManager.Self().GetGameActors().Actor(actor);
...
#else
        public async Task SetState(Image icon, string id) {
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
...
#else
            var tex = await UnityEditorWrapper.AssetDatabaseWrapper.LoadAssetAtPath<Texture2D>(path);
            icon.gameObject.SetActive(true);
#endif
            icon.sprite = ImageUtility.SpriteCreate(tex,
