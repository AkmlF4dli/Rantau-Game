...
#else
        public async Task Init(SkillMenu root, string skillId, Action openAction, Action closeAction) {
#endif
...
#else
            _actor = await DataManager.Self().GetGameActors().Actor(_skillMenu.GetActor());
#endif
...
#else
            _icon.sprite = await GetItemImage(_customDataModel.basic.iconId);
#endif
...
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
#else
            await gameAction.InitForConstructor(_actor);
#endif
...
#else
                await _skillMenu.MenuBase.OpenPartyWindow(PartyWindow.PartyType.Skill, _skillMenu.ActorId(), _customDataModel.basic.id, GameItem, _closeAction);
#endif
                _openAction?.Invoke();
