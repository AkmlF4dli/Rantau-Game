...
namespace RPGMaker.Codebase.Runtime.Common.Component.Hud
{
    public class Picture : MonoBehaviour
    {
        ...
        private Dictionary<int, byte[]> _pictureHitData;
        ...
        public async Task Init() {
            ...
            _pictureHitData = new Dictionary<int, byte[]>();
        ...
        public async Task AddPicture(int pictureNumber, string pictureName) {
            ...
            var sprite = ImageUtility.SpriteCreate(texture2D, new Rect(0, 0, texture2D.width, texture2D.height),
                Vector2.zero);
            ...
            var image = obj.GetComponent<CustomImage>();
            image.sprite = sprite;
            image.picture = this;
        ...
        {
            var image = _pictureList[pictureNumber].image;

            // クリック時に実行するコモンイベントがあるか
            if (_pictureName[pictureNumber].Count < 11 || (_pictureName[pictureNumber])[10] == "0")
            {
                // 以前のピクチャ番号でボタンをアタッチしている可能性があるので、あったら削除する
                if (image.TryGetComponent<Button>(out Button buttonComponent))
                {
                    Destroy(buttonComponent);
                }
                if (_pictureHitData.ContainsKey(pictureNumber))
                {
                    _pictureHitData.Remove(pictureNumber);
                }
            ...
            var executeCommonEventId = (_pictureName[pictureNumber])[10];
            ...
            // 透過画像の透過部分をクリックの判定にするかどうかのフラグを見てヒットデータを作成する。
            if (_pictureName[pictureNumber].Count >= 12 && (_pictureName[pictureNumber])[11] == "0" && image.sprite != null)
            {
                var hitData = GetHitData(_pictureList[pictureNumber]);
                if (_pictureHitData.ContainsKey(pictureNumber))
                {
                    _pictureHitData[pictureNumber] = hitData;
                } else
                {
                    _pictureHitData.Add(pictureNumber, hitData);
                }
            } else
            {
                if (_pictureHitData.ContainsKey(pictureNumber))
            ...
            button.onClick.AddListener(() => {
                ...
                // マウスクリックじゃなかったら何もしない
                if (!InputHandler.OnDown(Common.Enum.HandleType.LeftClick)) return;
#endif
                // イベント実行
                ...
                var eventCommand = new EventDataModel.EventCommand((int)EventEnum.EVENT_CODE_FLOW_JUMP_COMMON,
                    new List<string>() { executeCommonEventId }, new List<EventDataModel.EventCommandMoveRoute>(), 0);
                ...
                MapEventExecutionController.Instance.ExecuteCommonEventWithPicture(eventMapDataModel, eventDataModel);
            });
            button.transition = Selectable.Transition.None;
        }

        private byte[] GetHitData(PictureData pictureData) {
            var source = pictureData.image.sprite.texture;
            var width = source.width;
            var height = source.height;
            var hitData = new byte[(width + 8 - 1) / 8 * height];

            // 元のテクスチャをReadableに変換（新しいTexture2Dを生成）
            var readableTexture = new Texture2D(source.width, source.height, source.format, false, false);
            Graphics.CopyTexture(source, readableTexture);
            // 元テクスチャのピクセルデータを取得
            var pixels = readableTexture.GetPixels();
            var widthB = (width + 8 - 1) / 8;
            for (int j = 0; j < height; j++)
            {
                for (int i = 0; i < widthB; i++)
                {
                    ...
                    for (int k = 0; k < 8; k++)
                    {
                        if (i * 8 + k >= width) break;
                        if (pixels[i * 8 + k + j * width].a >= 0.1f) b |= (1 << k);
                    }
                    //if (b != 0) Debug.Log($"({i}, {j})");
                    hitData[i + j * widthB] = (byte)b;
                }
            }
        ...
        public async Task LoadPicture() {
            ...
            foreach (var data in DataManager.Self().GetRuntimeSaveDataModel().runtimeScreenDataModel.picture.pictureData)
            {
...
#else
                await AddPicture(pictureNumber, data.parameters[1]);
#endif
                //アンカー
...
#else
                await SetPosition(pictureNumber,
#endif
                ...
                // クリック時に実行するコモンイベントがあるか
                SetExecuteCommonEvent(pictureNumber);
            }
        ...
        public byte[] GetPictureHitData(Image image)
        {
            var item = _pictureList.FirstOrDefault(x => x.Value.image == image);
            var hitData = _pictureHitData.ContainsKey(item.Key) ? _pictureHitData[item.Key] : null;
            return hitData;
        }
        ...
        /// <summary>
        /// 画像の透過状況に合わせてIsRaycastLocationValidをオーバーライドするカスタムImage
        /// </summary>
        public class CustomImage : Image
        {
            public Picture picture;
            public override bool IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera) {
                var hitData = picture.GetPictureHitData(this);
                if (hitData == null)
                {
                    var baseHit = base.IsRaycastLocationValid(screenPoint, eventCamera);
                    return baseHit;
                }
                ...
                var textureCoord = new Vector2(
                    normalizedPoint.x * sprite.texture.width,
                    normalizedPoint.y * sprite.texture.height);

                var x = (int) textureCoord.x;
                var y = (int) textureCoord.y;
                var hit = (hitData[(x / 8) + y * ((sprite.texture.width + 8 - 1) / 8)] & (1 << (x % 8))) != 0;
