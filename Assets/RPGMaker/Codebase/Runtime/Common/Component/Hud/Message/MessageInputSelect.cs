    ...
    public class MessageInputSelect : MonoBehaviour
    {
        ...
        public void SetWindowColor(int kind) {
            switch (kind)
            {
                ...
                //暗くする
                case 1:
                    //U417 カラーを50%落として、暗くする様に変更
                    var frameColor = _frameImage.color * 0.5f;
                    _frameImage.color = new Color(frameColor.r, frameColor.g, frameColor.b,0.9f);
                    var backColor = _backgroundImage.color * 0.5f;
                    _backgroundImage.color = new Color(backColor.r, backColor.g, backColor.b,0.9f);
                    break;
                //透明
                case 2:
        ...
        public void ActiveSelectFrame(string text) {
            if (activeFrameCount < _ruckList.Count)
            {
                ...
                //U262 キャンバスを追加して、表示順を設定する
                var ImageObj = _ruckList[activeFrameCount].transform.Find("Box/Text/Image").gameObject;
                Canvas canVas = ImageObj.GetComponent<Canvas>();
                if (canVas == null)
                {
                    canVas = ImageObj.AddComponent<Canvas>();
                }
                if (canVas != null)
                {
