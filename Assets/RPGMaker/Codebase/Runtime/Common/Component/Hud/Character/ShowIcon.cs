...
#else
        private static async Task<List<Sprite>> CreateSpritesFromLoadBaloonIconTexture(
#endif
...
#else
            var texture = await UnityEditorWrapper.AssetDatabaseWrapper.LoadAssetAtPath<Texture2D>(
#endif
...
#else
                (await new DatabaseManagementService().LoadPopupIconAssets())
#endif
...
#else
            List<Sprite> animeSprites = await CreateSpritesFromLoadBaloonIconTexture(imageSetting);
#endif
