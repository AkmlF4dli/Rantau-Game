...
namespace RPGMaker.Codebase.Runtime.Common.ControlCharacter
{
    public class ControlCharacter : MonoBehaviour
    {
...
#else
        public async Task ExecCharacterByName() {
            await processer.ExecCharacterByName();
        }
#endif
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
        public void ExecEditor() {
#else
        public async Task ExecEditor() {
#endif
            NewProcessor();
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            processer.ExecEditor();
#else
            await processer.ExecEditor();
#endif
        }
...
#else
        public async Task ExecEditorByName() {
#endif
            NewProcessor();
#if (UNITY_EDITOR && !UNITE_WEBGL_TEST) || !UNITY_WEBGL
            processer.ExecEditorByName();
#else
            await processer.ExecEditorByName();
#endif
        }
        ...
        public class Processer
        {
...
#else
                    (_messageText, _charIndex) = await ExecParseChar(_messageText, _charIndex, _isAllSkip, isEditorPreview: false);
#endif
                }
            ...
            private async Task ExecFrameAsync()
            {
#endif
                DebugUtil.LogMethod();
...
#else
                    (_messageText, _charIndex) = await ExecParseChar(_messageText, _charIndex, _isAllSkip, isEditorPreview: true);
#endif
                }
            }
...
#else
            private async Task ParseRow(bool isSkipPxPyUntilPutObject, bool isEditorPreview) {
#endif
...
#else
                    (_messageText, _charIndex) = await ExecParseChar(_messageText, _charIndex, _isAllSkip, isEditorPreview);
#endif
                }
            ...
            private void ExecParseChar(
                ...
                ref int charIndex,
#else
            private async Task<(string, int)> ExecParseChar(
                string text,
                int charIndex,
#endif
                bool isAllSkip,
...
#else
                        rowInfo = await RowInfo.PreParseRow(this, isSkipPxPyUntilPutObject: true, isEditorPreview);
#endif
...
#else
                    rowInfo ??= await RowInfo.PreParseRow(this, isSkipPxPyUntilPutObject: false, isEditorPreview);
#endif
...
#else
                                    objectToProcess = await LoadIcon(GetParam(match));
#endif
...
#else
                                        objectToProcess = await LoadIcon(0);
#endif
                                    }
...
#else
                                    static async Task<Texture2D> LoadIcon(int iconNumber) {
            ...
            private float AddObjectComponent(GameObject go, object objectToPut) {
                ...
                if (objectToPut is Texture2D iconTexture)
                {
                    ...
                    image.sprite = ImageUtility.SpriteCreate(
                        iconTexture,
...
#else
                public static async Task<RowInfo> PreParseRow(
#endif
                    Processer processer,
...
#else
                    await clonedProcesser.ParseRow(isSkipPxPyUntilPutObject, isEditorPreview);
#endif
